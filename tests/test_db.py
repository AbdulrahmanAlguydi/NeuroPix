...
def run_tests():
    ...
    fake_orig_path = (
        f"https://neuropix-storage.s3.amazonaws.com/inputs/{unique_suffix}_raw.jpg"
    )
    fake_mod_path = (
        f"https://neuropix-storage.s3.amazonaws.com/outputs/{unique_suffix}_edited.jpg"
    )
    fake_edit_type = "ai"
    ...
    if register_user(test_username, test_password_hash):
        ...
        print(
            "    [FAILED] Registration query failed syntax or constraint verification."
        )
        sys.exit(1)
    ...
    if log_image_edit(target_user_id, fake_orig_path, fake_mod_path, fake_edit_type):
        print(
            "    [SUCCESS] Inserted image transaction into 'Images' table via foreign key link."
        )
    else:
        print(
            "    [FAILED] Image insertion failed. Check column names or ENUM constraints."
        )
        sys.exit(1)
    ...
    if len(user_gallery) > 0 and user_gallery[0]["ModifiedFilePath"] == fake_mod_path:
        print("    [SUCCESS] Selected historical stack from 'Images'.")
